...
- The number of trials is not fixed; trials continue until the first success occurs.
...
- We define $X$ to be the number of *failures* before the first success. So does R.

- OpenIntro defines $X$ to be the number of trials required to achieve the first success. This includes the success.
...
| Distribution | $f(x)$ in R | $F(x)$ in R |
| :--- | :--- | :--- |
| **Geometric** | `dgeom(x, prob)` | `pgeom(q, prob)` |
| **Binomial** | `dbinom(x, size, prob)` | `pbinom(q, size, prob)` |
